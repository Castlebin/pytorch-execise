# 11. 卷积神经网络(CNN)（高级篇）
## part 2. Residual Net 介绍
"""
Residual Net 主要是为了解决梯度消失的问题。增加了一个跳连接  
跳连接，H(x) = F(x) + x。 F(x)、x 两张量维度必须一样，加完后再激活
"""
...
# 定义网络结构
"""
先是 1 个卷积层(conv, maxpooling, relu)，然后 ResidualBlock 模块，
接下来又是一个卷积层(conv, mp, relu), 然后 residualBlock 模块，
最后一个全连接层(fc)。
"""
class Net(nn.Module):
    def __init__(self):
    ...
    def forward(self, x):
        ...
        h = self.mp(F.relu(self.conv1(x)))
        h = self.rblock1(h)
        
        h = self.mp(F.relu(self.conv2(h)))
        h = self.rblock2(h)
