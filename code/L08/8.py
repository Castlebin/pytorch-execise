# 8. 使用自定义的 Dataset 和 DataLoader 加载数据集、进行小批量训练 (Mini-Batch)
...
# 划分训练集和测试集
train_size = int(0.8 * len(dataset))  # 前 80% 作为训练集
test_size = len(dataset) - train_size  # 后 20% 作为测试集

# 只使用前 80% 的数据进行训练，后 20% 作为测试集
train_dataset, test_dataset = torch.utils.data.random_split(dataset, [train_size, test_size])

# dataloader - 用于小批量训练 (Mini-Batch)
...
class MultipleDimensionInputModel(torch.nn.Module):
    def __init__(self):
        ...
        self.activate_fn = torch.nn.Sigmoid()  # 激活函数。没有参数需要训练，所以可以共用一个

    def forward(self, x):
        h1 = self.activate_fn(self.linear1(x))
        h2 = self.activate_fn(self.linear2(h1))
        y_pred = self.activate_fn(self.linear3(h2))
        return y_pred
...
'''
遗憾的是，从结果看，模型训练时收敛得并不好，甚至可以说很差了
'''
